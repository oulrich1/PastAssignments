	...
	{	GalilPrivate * d;
		public:
			...
			static std::vector<std::string> addresses(); //returns list of available addresses to connect to (e.g. "1.2.3.4").  Each item in the list may be fed to the constructor Galil()
										 ...
										 Galil(std::string address = ""); //constructor opens connection with controller e.g. 192.168.1.2, COM1, /dev/ttyS0, GALILPCI1 /dev/galilpci0 (constructor must have an arguments)
